...
export const useRegisterForm = ({updateAuthorized, updateRegister}: LoginFormProps) => {
    ...
    const handlePassword1 = (event: any) => {
        ...
        if (!password1) {
            updateField('password1', { helperText: 'Введите пароль', error: true });
        } else {
            updateField('password1', { helperText: '', error: false });
        }
    
        if (fields['password2'].value && password1 !== fields['password2'].value) {
            updateField('password2', { helperText: 'Пароли не совпадают', error: true });
    ...
    const handlePassword2 = (event: any) => {
        ...
        if (!password2) {
            updateField('password2', { helperText: 'Подтвердите пароль', error: true });
    ...
    const handleCreateUser = (event: any) => {
        ...
        if (!fields['password1'].value || !fields['password2'].value) {
            updateField('password1', { helperText: 'Введите пароль', error: true });
            updateField('password2', { helperText: 'Повторите пароль', error: true });
            errorEmpty = true;
        }
        else {
            updateField('password1', { helperText: '', error: false });
